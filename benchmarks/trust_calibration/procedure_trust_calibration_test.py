...
Dogfood mode reads committed evidence/dogfood/results, legacy dogfood-results,
and local dogfood Codex metadata:
...
DEFAULT_DOGFOOD_RESULTS_ROOT = Path("evidence/dogfood/results")
LEGACY_DOGFOOD_RESULTS_ROOT = Path("dogfood-results")
STATUS_ORDER = (
    "verified",
    "candidate",
    "stale",
...
def load_dogfood_samples(
    *,
    results_root: Path = DEFAULT_DOGFOOD_RESULTS_ROOT,
    ...
    root = _resolve_results_root(Path(results_root))
...
def main() -> int:
    ...
    if dry_run or not source:
        ...
        summary = dogfood_summary(
            results_root=Path(
                os.getenv(
                    "HOWDEX_CALIBRATION_RESULTS_DIR",
                    str(DEFAULT_DOGFOOD_RESULTS_ROOT),
                )
            ),
            codex_dir=Path(
                os.getenv(
...
def _resolve_results_root(results_root: Path) -> Path:
    """Resolve committed dogfood evidence while preserving legacy local output."""
    root = Path(results_root)
    if root.is_dir():
        return root
    if (
        root == DEFAULT_DOGFOOD_RESULTS_ROOT
        and LEGACY_DOGFOOD_RESULTS_ROOT.is_dir()
    ):
        return LEGACY_DOGFOOD_RESULTS_ROOT
    return root
