...
demo_dir = Path.home() / ".howdex-openai-agents-demo-v2"
...
# This is the key fix.
# We do not scrape the SDK result object and guess tool names.
# Each tool records itself when it actually runs.
CURRENT_TRACE: list[dict[str, str]] = []


def record_tool(name: str, output: str) -> str:
    CURRENT_TRACE.append({"name": name, "output": str(output)})
    return output
...
def inspect_howdex() -> str:
    ...
    return record_tool("inspect_howdex", howdex_context())
...
def check_DATABASE_URL() -> str:
    ...
    result = "present" if "DATABASE_URL=" in text else "missing"
    return record_tool("check_DATABASE_URL", result)
...
def check_migration_file() -> str:
    ...
    result = "present" if text else "empty"
    return record_tool("check_migration_file", result)
...
def run_agent_attempt(attempt_name: str):
    ...
    outcome, error = determine_outcome(CURRENT_TRACE, final_output)
...
print("\n🧠 Howdex + OpenAI Agents SDK Demo v2")
print("=====================================")
print(f"Model: {MODEL}")
print("This uses OpenAI Agents SDK with real function tools and clean tool-level instrumentation.\n")
...
if proc and proc.steps:
    print("\n✅ Howdex learned a clean OpenAI-Agents-SDK deployment procedure:")
    learned = []
    for i, step in enumerate(proc.steps, start=1):
        action = step.get("action", step) if isinstance(step, dict) else step
    ...
    if "unknown" in learned:
        ...
        print("\n✅ PASS: Procedure contains only real executable task actions.")
...
print("   function tools execute → Howdex remembers → Howdex learns → future attempts improve.")
